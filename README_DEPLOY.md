...
Protótipo encriptado (StaticCrypt AES-256, roda 100% no navegador). Senha: `montelibano-pass-2026`

## Passos (Gustavo)
1. Criar repo em github.com/new. Como o índice é encriptado, pode ser **público** (necessário para GitHub Pages no plano Free).
2. Subir os arquivos desta pasta (index.html, CNAME, .nojekyll, MonteLibano_Prototipo.html) para a raiz do repo (branch main).
3. Ativar GitHub Pages (Settings → Pages → branch main / root) e aguardar o certificado HTTPS provisionar.
4. DNS no **Wix**: Domínios → guilds.com.br → Registros DNS → CNAME host `montelibano` valor `SEU_USUARIO.github.io`.
...
> Oi Monte Líbano! Preparei uma prévia navegável do sistema. Acesse https://montelibano.guilds.com.br e use a senha `montelibano-pass-2026`. Dá uma olhada com calma e me conta o que achou 🤝
